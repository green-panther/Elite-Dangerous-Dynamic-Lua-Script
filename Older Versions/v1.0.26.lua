...
----------------------------------------------------------------------------------------------
		elseif (EIP == table['damageCalc'].addr) then
			...
			if (caller == table['shieldCaller'].addr) then
				...
				local flval = hex2float(xoredval)
				local hullPtr = readAddress(hullRefPtr)
				local npcFlag = isNPC(hullPtr)
				local authFlag = isAUTH(hullPtr)
				
				--printDBG(string.format("Shield | shieldModulePtr: 0x%X - hullRefPtr: 0x%X - hullPtr: 0x%X - Shield Value: %.2f", shieldModulePtr, hullRefPtr, hullPtr, flval))
