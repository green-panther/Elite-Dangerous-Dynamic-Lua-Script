...
function printDBG(string)
	if (table['debug'].on == true or debugmode == true) then
		f = assert(io.open(logfilename, "a+"))
...
table.udpStream = { descr = "UPD Stream Logger", sig = "83 EC 20 53 8B D9 55 8B 53 54", force = true }
...
function script.main()
	...
	if (foundError == true) then
		...
		for k in ordered(table) do
			if ((table[k].enableOnStart == 1) or (force == true)) then
				if (table[k].descr ~= nil) then
					toggle(k, true)

				elseif (table[k].patch == nil) then
...
function script.onBreakpoint()
	...
	elseif (EIP == table['udpStream'].addr) then
		...
		if (streamTimer == nil) then
			...
			timer_onTimer(streamTimer, function() if (streamSizePerSecond ~= 0) then printDBG('UDP Stream | ' .. streamSizePerSecond .. ' Bytes/s - ' .. streamCountPerSecond .. ' Packets/s') streamSizePerSecond = 0 streamCountPerSecond = 0 end end)
		end
		...
		if (streamSize ~= nil and streamCount ~= nil) then
			...
			streamSize = size
			streamCount = 1
			printStreamStats = function() printDBG(string.format("Total Size: %i Bytes | Packet Count: %i Packets", streamSize, streamCount)) end
		end

		if (streamSizePerSecond > 20000) then
			printDual('Warning! Your traffic is unusually high. Please report this to Randshot! (' .. streamSizePerSecond .. ' Bytes/s - ' .. streamCountPerSecond .. ' Packets/s)')
			beep()
		end
