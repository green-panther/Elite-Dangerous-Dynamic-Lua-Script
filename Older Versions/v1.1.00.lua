...
table.damageSrc 		= { idx = 4, descr = "Only You", 				pat = "55 8B EC 83 E4 F0 81 EC ? ? ? ? 56 8B F1 57 8B 86 ? ? ? ? 8D 8E ? ? ? ?", child = "damageBoost", hotkey = hDamageSource, default = 1 }
...
table.scoop   			= { idx = 33, descr = "Instant scooping",		pat = "80 BC 24 ? ? ? ? ? 0F 84 ? ? ? ? 8B 5C 24 2C", hotkey = hScoop, default = 0 }
...
----------------------------------------------------------------------------------------------
	elseif (EIP == table['damageCalc'].addr) then
		...
		elseif (caller == table['moduleCaller'].addr) then
			-- Temporary till the new offsets are figured out
			writeFloat(result, 0.0)
			damageDoneTo = "Temporary | Module"
 
		--[[
			local modulesModulePtr = pthis + 324 - 136
			local hullRefPtr = modulesModulePtr + 100
			local hullPtr = readAddress(hullRefPtr)
			local npcFlag = isNPC(hullPtr)
 
			printDBG(string.format("hullPtr: 0x%X | npcFlag: %s", hullPtr, tostring(npcFlag)))
 
			--printDBG(string.format("Module | hullPtr: 0x%X", hullPtr))
 
			if ((npcFlag == false) and (table['noModD'].on == true)) then
				writeFloat(result, 0.0)
...
----------------------------------------------------------------------------------------------
	elseif (EIP == table['hyper'].addr) then
		...
		--printDBG(string.format("HyperJump | maxFuelUsageVal: %.2f - toTargetFuelUsageVal: %.2f - currentFuelAvailableVal: %.2f", maxFuelUsageVal, toTargetFuelUsageVal, currentFuelAvailableVal))
		...
		if ((maxFuelUsageVal ~= nil) and (toTargetFuelUsageVal ~= nil) and (currentFuelAvailableVal ~= nil)) then
			...
			elseif (table['hyFuelUsage'].on == true) then
				if (toTargetFuelUsageVal * 0.1 < currentFuelAvailableVal) then
				...
				elseif (toTargetFuelUsageVal * 0.01 < currentFuelAvailableVal) then
					writeFloat(toTargetFuelUsageAddr, toTargetFuelUsageVal * 0.01)
					printDBG(string.format("Reduced fuel usage by a factor of %.2f!", 1 / 0.01))
 
				elseif (toTargetFuelUsageVal * 0.001 < currentFuelAvailableVal) then
					writeFloat(toTargetFuelUsageAddr, toTargetFuelUsageVal * 0.001)
					printDBG(string.format("Reduced fuel usage by a factor of %.2f!", 1 / 0.001))
				end
			end
 
			-- Jump Range Check
			if (table['hyFuelRange'].on == true) then
				local newMaxFuelUsageVal = maxFuelUsageVal * maxFuelMultiplier
				writeFloat(maxFuelUsageAddr, newMaxFuelUsageVal)
 
				if (currentFuelAvailableVal < newMaxFuelUsageVal) then
					writeFloat(currentFuelAvailableAddr, newMaxFuelUsageVal)
				end
...
function waitForWD(timer)
	if (getProcessIDFromProcessName(script.wd) ~= nil) then
		object_destroy(timer)
 
		script.edPID = getProcessIDFromProcessName(script.ed)
		script.wdPID = getProcessIDFromProcessName(script.wd)
 
		printDual("Found the process WatchDog.exe! Now waiting "..secToWait.." seconds...")
