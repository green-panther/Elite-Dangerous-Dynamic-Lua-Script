...
changelog = function(version)
  ...
  changeTable['v2.0.04'] = [[
    List of changes for v2.0.04:
      ...
      Added: Changelog outputting (automatically detects new versions)
      Added: Use printChangelog() to output entire changelog
  ]]
  changeTable['v2.0.03'] = [[
    List of changes for v2.0.03:
      Added: Interdiction Bypass
  ]]
  changeTable['v2.0.02'] = [[
    List of changes for v2.0.02:
      Changed: I changed the way the data gets downloaded, so there are only two popups when it updates the data!
      Changed: Implemented a folder check for the EliteOut directory. There shouldn't be popups if the folder already exists.
  ]]
  changeTable['v2.0.01'] = [[
    List of changes for v2.0.01:
      Fixed: Just a small fix regarding debug output.
  ]]
  changeTable['v2.0.00'] = [[
    List of changes for v2.0.00:
      Changed: Most of the code will now be retrieved from my GitHub repository
      Added: This version includes a prototype version of the planned auto pilot
  ]]
  ...
  if (type(version) == 'string') then
    ...
    local changelist = ""
    for v in ordered(changeTable) do
      changelist = string.format("%s\n%s", changelist, changeTable[v])
    end
...
ctable = function()
  ...
  ctable.interdictionBypass = { descr = "Interdiction Bypass", patch = { "0F 82 ? ? ? ? 8B 07 8B CF 8B 40 10", "90 E9" }, hotkey = nil }
  ...
  ctable.voucher = { descr = "Voucher Multiplier", hook = "81 EC ? ? ? ? 8B 84 24 ? ? ? ? 53 8B 50 0C", hide = true }
  ctable.voucherMul = { descr = "Multiplier", customAddr = "vMul", parent = "voucher", hotkey = nil, hide = true }
end,
